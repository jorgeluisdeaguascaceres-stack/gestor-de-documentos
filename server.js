...
function cookieSesion(token, borrar){
...
function autorizado(req,res){
  if(!hayUsuarios()) return true;        // sin usuarios creados: modo abierto (se avisa en consola)
  var s = sesionDe(req);
...
http.createServer(async function(req,res){
  ...
  if(p === "/api/login" && req.method === "POST"){
    ...
    if(!hayUsuarios()) return json(res,200,{ ok:true, abierto:true });
    ...
    res.setHeader("Set-Cookie", cookieSesion(token, false));
  ...
  if(p === "/api/logout" && req.method === "POST"){
    ...
    res.setHeader("Set-Cookie", cookieSesion("", true));
